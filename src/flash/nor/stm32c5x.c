 ...
 * Adapted for STM32C5 by liptakgabor@freemail.hu
 */
...
static const uint32_t stm32c5x_ns_flash_regs[STM32_FLASH_REG_INDEX_NUM] = {
...
static const uint32_t stm32c5x_s_flash_regs[STM32_FLASH_REG_INDEX_NUM] = {
	[STM32_FLASH_ACR_INDEX]			= 0x000,
...
static int stm32c5x_set_secbb(struct flash_bank *bank, uint32_t value)
{
	/* This function should be used only with device with TrustZone, do just a security check */
	struct stm32c5x_flash_bank *stm32c5x_bank = bank->driver_priv;
	assert(stm32c5x_bank->dev_info->flags & F_HAS_TZ);

	int retval = stm32c5x_wait_status_prev_op(bank, FLASH_TIMEOUT);
	if (retval != ERROR_OK)
		return retval;
	...
	for (unsigned int i = 0; i < num_secbb_regs; i++) {
		retval = stm32c5x_write_flash_reg(bank, secbb_regs[i], value);
		if (retval != ERROR_OK)
			return retval;
	}
	...
	uint32_t value, uint32_t mask)
{
	...
	/* for STM32C5 and similar devices, use always non-secure
	 * registers for option bytes programming */
	const uint32_t *saved_flash_regs = stm32c5x_bank->flash_regs;
	if (stm32c5x_bank->dev_info->flags & F_HAS_TZ)
		stm32c5x_bank->flash_regs = stm32c5x_ns_flash_regs;
...
err_lock:
	...
	stm32c5x_bank->flash_regs = saved_flash_regs;
		...
		unsigned int last)
{
	...
	if (stm32c5x_bank->tzen == TZEN_ENABLED && stm32c5x_bank->pstate == PSTATE_OPEN) {
		/* set all FLASH pages as secure */
		retval = stm32c5x_set_secbb(bank, FLASH_SECBB_SECURE);
		if (retval != ERROR_OK) {
			/* restore all FLASH pages as non-secure */
			stm32c5x_set_secbb(bank, FLASH_SECBB_NON_SECURE); /* ignore the return value */
			return retval;
		}
	}
	...
	for (unsigned int i = first; i <= last; i++) {
		...
		if (i >= stm32c5x_bank->bank1_sectors) {
			uint8_t snb;
			snb = i - stm32c5x_bank->bank1_sectors;
...
err_lock:
	...
	if (stm32c5x_bank->tzen == TZEN_ENABLED && stm32c5x_bank->pstate == PSTATE_OPEN) {
		/* restore all FLASH pages as non-secure */
		int retval3 = stm32c5x_set_secbb(bank, FLASH_SECBB_NON_SECURE);
		if (retval3 != ERROR_OK)
			return retval3;
	}
...
static int stm32c5x_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	...
	return stm32c5x_protect_inner(bank, set, STM32_BANK1,
...
#include "../../../contrib/loaders/flash/stm32/stm32c5x.inc"
	...
	/* buffer_size should be multiple of stm32c5_info->data_width */
	...
	init_reg_param(&reg_params[0], "r0", 32, PARAM_IN_OUT);	/* stm32c5_work_area ptr , status (out) */
	init_reg_param(&reg_params[1], "r1", 32, PARAM_OUT);	/* buffer end */
	init_reg_param(&reg_params[2], "r2", 32, PARAM_OUT);	/* target address */
	init_reg_param(&reg_params[3], "r3", 32, PARAM_OUT);	/* count (of stm32c5_info->data_width) */
	...
	if (retval == ERROR_FLASH_OPERATION_FAILED) {
		...
		if (error != 0) {
			...
			/* Clear but report errors */
			stm32c5x_write_flash_reg_by_index(bank, STM32_FLASH_SR_INDEX, error);
			retval = ERROR_FAIL;
		}
	}
				...
				uint32_t offset, uint32_t count)
{
	...
	retval = stm32c5x_write_flash_reg_by_index(bank, STM32_FLASH_CR_INDEX, 0);
	if (retval != ERROR_OK)
		return retval;
	...
	uint32_t offset, uint32_t count)
{
	...
	if (stm32c5x_bank->tzen == TZEN_ENABLED && stm32c5x_bank->pstate != PSTATE_OPEN) {
		...
		retval = stm32c5x_write_block_slow(bank, buffer, offset, count / FLASH_DATA_WIDTH);
	} else {
...
int stm32c5x_read(struct flash_bank *bank,
	uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	if (stm32c5x_bank->tzen == TZEN_ENABLED && stm32c5x_bank->pstate == PSTATE_OPEN) {
		/* restore all FLASH pages as non-secure */
		retval = stm32c5x_set_secbb(bank, FLASH_SECBB_NON_SECURE);
		if (retval != ERROR_OK)
			return retval;
	}
...
static void stm32c5x_sync_product_state(struct flash_bank *bank)
{
	struct stm32c5x_flash_bank *stm32c5x_bank = bank->driver_priv;

	stm32c5x_bank->tzen = TZEN_DISABLED;
	if (stm32c5x_bank->dev_info->flags & F_HAS_TZ)
		stm32c5x_bank->tzen = (stm32c5x_bank->optsr2_cur >> FLASH_TZEN_POS) & FLASH_TZEN_MASK;
...
static int stm32c5x_probe(struct flash_bank *bank)
{
	...
	const char *device_str = get_stm32c5x_device_str(bank);
	const char *rev_str = get_stm32c5x_rev_str(bank);
	const uint16_t rev_id = stm32c5x_bank->idcode >> 16;

	LOG_INFO("device idcode = 0x%08" PRIx32 " (%s - Rev %s : 0x%04x)",
			stm32c5x_bank->idcode, device_str, rev_str, rev_id);
	...
	stm32c5x_sync_product_state(bank);

	/* For devices with TrustZone enabled and Product State 'Open', use flash secure registers */
	if (stm32c5x_bank->tzen == TZEN_ENABLED && stm32c5x_bank->pstate == PSTATE_OPEN) {
		stm32c5x_bank->flash_regs_base |= STM32_REGS_SEC_OFFSET;
		stm32c5x_bank->flash_regs = stm32c5x_s_flash_regs;
	}

	if (dev_info->flags & F_HAS_TZ)
		LOG_INFO("TZEN = 0x%02X : TrustZone %s by option bytes",
				stm32c5x_bank->tzen,
				stm32c5x_bank->tzen == TZEN_ENABLED ? "enabled" : "disabled");
	...
	stm32c5x_bank->dual_bank = false;
	stm32c5x_bank->bank1_sectors = 0;
	int num_sectors = 0;
	...
	case DEVID_STM32C5XX:
		/* Always dual-bank */
		stm32c5x_bank->dual_bank = true;
		sector_size_kb = 8;
		num_sectors = flash_size_kb / sector_size_kb;
		stm32c5x_bank->bank1_sectors = num_sectors / 2;
		break;
	default:
	...
	stm32c5x_bank->bank1_prot_blocks = stm32c5x_bank->bank1_sectors;
	...
	stm32c5x_bank->bank1_prot_blocks = bank->num_prot_blocks / 2;
...
static int stm32c5x_auto_probe(struct flash_bank *bank)
{
	...
	if (stm32c5x_bank->probed) {
		...
		if (stm32c5x_bank->optsr_cur == optsr_cur)
			return ERROR_OK;
...
COMMAND_HANDLER(stm32c5x_handle_mass_erase_command)
{
	...
	retval = stm32c5x_mass_erase(bank, STM32_ALL_BANKS);
	if (retval == ERROR_OK)
...
COMMAND_HANDLER(stm32c5x_handle_trustzone_command)
{
	/* TODO */
	return ERROR_FLASH_OPER_UNSUPPORTED;
}

COMMAND_HANDLER(stm32c5x_handle_option_load_command)
{
	/* TODO */
	return ERROR_FLASH_OPER_UNSUPPORTED;
}

COMMAND_HANDLER(stm32c5x_write_obk)
{
	struct flash_bank *bank;
	int retval = CALL_COMMAND_HANDLER(flash_command_get_bank, 0, &bank);
	if (retval != ERROR_OK)
	...
	struct rsslib_data_provisioning_conf pconf = {
		.p_source = stm32c5x_bank->available_rm_addr + 0x18,
	...
	const uint32_t return_addr = stm32c5x_bank->available_rm_addr;
	target_write_u32(target, stm32c5x_bank->available_rm_addr + 0x4, stm32c5x_bank->available_rm_addr + 0x18);
	target_write_u32(target, stm32c5x_bank->available_rm_addr + 0x8, pconf.p_destination);
	target_write_u32(target, stm32c5x_bank->available_rm_addr + 0xC, pconf.size);
	target_write_u32(target, stm32c5x_bank->available_rm_addr + 0x10, pconf.do_encryption);
	target_write_u32(target, stm32c5x_bank->available_rm_addr + 0x14, pconf.crc);
	...
	buf_set_u32(reg_params[0].value, 0, 32, stm32c5x_bank->available_rm_addr + 0x4);

	breakpoint_add(target, return_addr, 2, BKPT_SOFT);
	retval = target_run_algorithm(target,
				0, NULL,
				ARRAY_SIZE(reg_params), reg_params,
				stm32c5x_bank->rss_lib_addr,
...
COMMAND_HANDLER(stm32c5x_write_obk_options)
{
	if (CMD_ARGC < 2) {
	...
	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[1], stm32c5x_bank->available_rm_addr);
	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[2], stm32c5x_bank->rss_lib_addr);
...
static const struct command_registration stm32c5x_exec_command_handlers[] = {
	...
	{
		.name = "trustzone",
		.handler = stm32c5x_handle_trustzone_command,
		.mode = COMMAND_EXEC,
	...
	{
		.name = "option_load",
		.handler = stm32c5x_handle_option_load_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
	...
	},
	{
		...
		.handler = stm32c5x_write_obk_options,
		.mode = COMMAND_EXEC,
